...
void
framebuffer_size_callback(GLFWwindow *window, uint16_t width, uint16_t height);
void
processInput(GLFWwindow *window);
int
getMaxVertexAttribs();// return GL_MAX_VERTEX_ATTRIB
...
main(int argc, char *argv[])
{
	...
	Shader textureShader
		("C:/Dev/Graphics/resources/shaders/twoTextures.vs", "C:/Dev/Graphics/resources/shaders/twoTextures.fs");
	...
	// TexCoord -> Enable 2 because in shader program it's declared ourColour too
	...
	glBindTexture(GL_TEXTURE_2D, texture);// all upcoming GL_TEXTURE_2D operations have effect on this texture object
	...
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);	// GL_FILL -> normal. GL_LINE -> wireframe mode
	glEnable(GL_DEPTH_TEST); // Need to clear GL_DEPTH_BUFFER_BIT every render loop
...
//	model = glm::rotate(model, glm::radians(-55.0f), glm::vec3(1.0f, 0.0f, 0.0f));
	...
	view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f)); // Is like moving all the model from you
...
//	textureShader.setMatrix4fv("model", glm::value_ptr(model)); // Set model
	textureShader.setMatrix4fv("view", glm::value_ptr(view)); // Set view
	textureShader.setMatrix4fv("projection", glm::value_ptr(projection)); // Set projection
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// clear back buffer. COLOR_BUFFER and DEPTH_BUFFER
		...
		{
			// every transformation should be created in this scope if not, it will be incremented in every loop.
			auto model = glm::mat4(1.0f);
			...
			textureShader.setMatrix4fv("model", glm::value_ptr(model));
